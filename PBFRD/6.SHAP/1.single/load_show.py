...
for i in range(len(shap_interaction_values)):
	...
	"""
	if train_X[features[xxx]][train_X.index[i]] >= 1.01:
		new_X.append(train_X[features[xxx]][train_X.index[i]])
		new_shap.append(shap_interaction_values[i][xxx][xxx])
	if train_X[features[xxx]][train_X.index[i]] <= 1.02:
...
plt.subplots_adjust(left=0.25, bottom=0.2, right=0.8, top=0.85, wspace=0, hspace=0)
ax.tick_params(width=2, length=8, pad=12)
plt.rcParams.update({'font.size': 42})

#plt.scatter(new_X3, new_shap3, color='black', alpha = 0.25, s = 30)
sns.regplot(x=features[xxx],y='shap', lowess=True, data=new_df, line_kws={'color': 'black', 'linewidth':5}, scatter_kws={'alpha':0.25, 'color': 'black', 's':30}, ax=ax)
#sns.regplot(x=features[xxx],y='shap', lowess=True, data=new_df, line_kws={'color': 'black', 'linewidth':5}, ax=ax, scatter = False)# scatter_kws={'alpha':0.25, 'color': 'black', 's':30}, ax=ax)
#sns.regplot(x=features[xxx],y='shap', lowess=True, data=new_df2, line_kws={'color': 'black', 'linewidth':5}, ax=ax, scatter = False)# scatter_kws={'alpha':0.25, 'color': 'black', 's':30}, ax=ax)

plt.xlabel(features_n[xxx] + ' (' + features_u[xxx] + ')')
plt.ylabel('SHAP Main Value')
...
plt.xticks([0.13, 0.6, 1.06])#np.arange(xmin, xmax+0.01, xint))
plt.yticks(np.arange(ymin, ymax+0.01, yint))
plt.hlines(0, xmin, xmax, color = 'black', linestyle='--', linewidth=3)
for item in ([ax.title, ax.xaxis.label, ax.yaxis.label]):
	item.set_fontsize(66) #33
for item in ([] + ax.get_xticklabels() + ax.get_yticklabels()):
	item.set_fontsize(54) #27
plt.show()
#plt.savefig('main_' + features_s[xxx] + '.png')
