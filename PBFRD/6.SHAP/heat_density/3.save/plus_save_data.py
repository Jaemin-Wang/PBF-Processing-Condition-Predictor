...
features = ['reflectivity (%)','thermal conductivity (W/mK)','specific heat capacity (J/cm3K)','melting point (C)','laser power (watt)','scan speed (mm/s)','layer thickness (mm)','hatch spacing (mm)','energy density (J/mm3)','relative density (%)']
...
features_u = ['%','W/mK','J/cm$^3$K','$^o$C','W','mm/s','mm','mm','J/mm$^3$']
features_s = ['reflect','cond','heat','melt','power','speed','thick','hatch','energy']
...
for i in range(len(shap_interaction_values)):
	...
	if train_X[infp][train_X.index[i]] >= topl:
		...
		new_shap.append(shap_interaction_values[i][xxx][xxx] + shap_interaction_values[i][xxx][zzz] * 2)
	#elif train_X[infp][train_X.index[i]] >= botl: 
	#	new_X2.append(train_X[features[xxx]][train_X.index[i]])
	#	new_shap2.append(shap_interaction_values[i][xxx][xxx] + shap_interaction_values[i][xxx][zzz] * 2)
	else:
		new_X3.append(train_X[features[xxx]][train_X.index[i]])
		new_shap3.append(shap_interaction_values[i][xxx][xxx] + shap_interaction_values[i][xxx][zzz] * 2)
...
conpd.to_csv('plus_' + features_s[xxx] + '_' + features_s[zzz] + '.csv', index=False)
